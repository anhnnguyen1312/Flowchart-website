...
const TableForm: FC<TableFormProps> = ({ value, onChange, id, properties, setProperties }) => {
  ...
  const remove = (key: string) => {
    const newData = properties?.filter((item) => item.key !== key) as TableFormDateType[];
...
e.persist();
    ...
    setTimeout(() => {
      ...
      if (!target.name || !target.property) {
        ...
        if (record.editable) {
          return (
            <Input
              ...
              onChange={(e) => handleFieldChange(e, 'property', record.key)}
              onKeyPress={(e) => handleKeyPress(e, record.key)}
      ...
      render: (text: string, record: TableFormDateType) => {
        if (record.editable) {
          return (
            ...
            onChange={(value) => handleSelectChange(value, 'type', record.key)}
              options={[
              {
                value:"",
                label:"Choose type"
        ...
        if (record.editable) {
          if (record.isNew) {
            return (
              <span>
                ...
                <a type={"button"} className={"btn btn-success"} onClick={(e) => saveRow(e, record.key)}>Lưu</a>
                <Divider type="vertical" /> 
            
                  <a type={"button"} className={"btn btn-danger"} onClick={() => remove(record.key)}>Huỷ</a>
          ...
          return (
            <span>
              <a type={"button"} className={"btn btn-success"} onClick={(e) => saveRow(e, record.key)}>Lưu</a>
              <Divider type="vertical" />
              <a type={"button"} className={"btn btn-danger"} onClick={(e) => cancel(e, record.key)}>Huỷ</a>
            </span>
          );
        }
        return (
          <span>
            <a type={"button"} className={"btn btn-primary"} onClick={(e) => toggleEditable(e, record.key)}>Cập nhật</a>
            <Divider type="vertical" />
            <Popconfirm title="Bạn có muốn xoá dữ liệu này?" onConfirm={() => remove(record.key)}>
              <a type={"button"} className={"btn btn-danger"}>Xoá</a>
            </Popconfirm>
          </span>
        );
      },
